...
void enter_matrix_size(int &width_of_matrix,int &hight_of_matrix,int index_of_matrix)
{
    do
    {
        cout << "Введите размеры матрицы № "<< index_of_matrix << " (значение большьше 0)\n";
        cin >> width_of_matrix >> hight_of_matrix;
    } while (width_of_matrix <= 0 || hight_of_matrix <= 0);
}

void fill_of_matrix(int** first_matrix,int** second_matrix, int width_of_first_matrix, int hight_of_first_matrix,
                         int width_of_second_matrix, int hight_of_second_matrix)
{
    ...
    if (matrix_filling_mode == 1)
    {
        for (int i = 0; i < width_of_first_matrix; i++)
            for (int j = 0; j < hight_of_first_matrix; j++)
                cin >> first_matrix[i][j];
        for (int i = 0; i < width_of_second_matrix; i++)
            for (int j = 0; j < hight_of_second_matrix; j++)
...
void output_matrix(int** matrix, int width_of_matrix, int hight_of_matrix)
{
    for (int i = 0; i < width_of_matrix; i++)
    {
        for (int j = 0; j < hight_of_matrix; j++)
            cout << matrix[i][j] << " ";
        cout << endl;
    }
}

void bring_matrix(int** new_matrix, int** old_matrix,int size_of_new_matrix, int width_of_old_matrix, int hight_of_old_matrix)
{
    ...
    {
        for (int j = 0; j < hight_of_old_matrix; j++)
...
{
    int width_of_first_matrix, hight_of_first_matrix,
        width_of_second_matrix, hight_of_second_matrix,
    ...
    ///////////////////////////////////////////////////////////////////////////////
    enter_matrix_size(width_of_first_matrix, hight_of_first_matrix, 1);
    enter_matrix_size(width_of_second_matrix, hight_of_second_matrix, 1);

    int** first_matrix = new int* [width_of_first_matrix];
    int** second_matrix = new int* [width_of_second_matrix];
    for (int i = 0; i < width_of_first_matrix; i++)
        first_matrix[i] = new int[hight_of_first_matrix];

    for (int i = 0; i < width_of_second_matrix; i++)
        second_matrix[i] = new int[hight_of_second_matrix];
    ...
    fill_of_matrix(first_matrix,second_matrix,width_of_first_matrix,
                        hight_of_first_matrix,width_of_second_matrix, hight_of_second_matrix);

    cout << "\nМатрица 1\n\n";
    output_matrix(first_matrix, width_of_first_matrix, hight_of_first_matrix);
    cout << "\nМатрица 2\n\n";
    output_matrix(second_matrix, width_of_second_matrix, hight_of_second_matrix);

    ///////////////////////////////////////////////////////////////////////////////
    /////////////////Приведение матриц к требуемому размеру////////////////////////
    ///////////////////////////////////////////////////////////////////////////////

    while (size_of_intermediate_matrix < width_of_first_matrix || size_of_intermediate_matrix < hight_of_first_matrix
        || size_of_intermediate_matrix < width_of_second_matrix || size_of_intermediate_matrix < hight_of_second_matrix)
        size_of_intermediate_matrix *= 2;
    int** therd_matrix = new int* [size_of_intermediate_matrix];
    bring_matrix(therd_matrix, first_matrix, size_of_intermediate_matrix, width_of_first_matrix, hight_of_first_matrix);
    int** fouth_matrix = new int* [size_of_intermediate_matrix];
    bring_matrix(fouth_matrix, second_matrix, size_of_intermediate_matrix, width_of_second_matrix, hight_of_second_matrix);

    cout << "Приведенные матрицы\n";
    cout << "\nМатрица 1\n\n";
    output_matrix(therd_matrix, size_of_intermediate_matrix, size_of_intermediate_matrix);
    cout << "\nМатрица 2\n\n";
    output_matrix(fouth_matrix, size_of_intermediate_matrix, size_of_intermediate_matrix);

    ///////////////////////////////////////////////////////////////////////////////
    ///////////////Разбиение матриц на подматрицы и их заполнение//////////////////
    ///////////////////////////////////////////////////////////////////////////////

    int** podmatrix_1_of_therd = new int* [size_of_intermediate_matrix / 2];
    int** podmatrix_2_of_therd = new int* [size_of_intermediate_matrix / 2];
    int** podmatrix_3_of_therd = new int* [size_of_intermediate_matrix / 2];
    int** podmatrix_4_of_therd = new int* [size_of_intermediate_matrix / 2];
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_1_of_therd, therd_matrix, 0, 0);
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_2_of_therd, therd_matrix, 0, size_of_intermediate_matrix / 2);
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_3_of_therd, therd_matrix, size_of_intermediate_matrix / 2, 0);
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_4_of_therd, therd_matrix, size_of_intermediate_matrix / 2, size_of_intermediate_matrix / 2);

    int** podmatrix_1_of_fouth = new int* [size_of_intermediate_matrix / 2];
    int** podmatrix_2_of_fouth = new int* [size_of_intermediate_matrix / 2];
    int** podmatrix_3_of_fouth = new int* [size_of_intermediate_matrix / 2];
    int** podmatrix_4_of_fouth = new int* [size_of_intermediate_matrix / 2];
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_1_of_fouth, fouth_matrix, 0, 0);
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_2_of_fouth, fouth_matrix, 0, size_of_intermediate_matrix / 2);
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_3_of_fouth, fouth_matrix, size_of_intermediate_matrix / 2, 0);
    copy_values_to_matrix(size_of_intermediate_matrix / 2, podmatrix_4_of_fouth, fouth_matrix, size_of_intermediate_matrix / 2, size_of_intermediate_matrix / 2);
    ...
    for (int i = 0; i < size_of_intermediate_matrix / 2; i++)
    {
        for (int j = 0; j < size_of_intermediate_matrix / 2; j++)
        {
            ...
            {
                intermediate_matrix_1[i][j] += (podmatrix_1_of_therd[i][z] + podmatrix_4_of_therd[i][z]) * (podmatrix_1_of_fouth[z][j] + podmatrix_4_of_fouth[z][j]);
                intermediate_matrix_2[i][j] += (podmatrix_3_of_therd[i][z] + podmatrix_4_of_therd[i][z]) * podmatrix_1_of_fouth[z][j];
                intermediate_matrix_3[i][j] += podmatrix_1_of_therd[i][z] * (podmatrix_2_of_fouth[z][j] - podmatrix_4_of_fouth[z][j]);
                intermediate_matrix_4[i][j] += podmatrix_4_of_therd[i][z] * (podmatrix_3_of_fouth[z][j] - podmatrix_1_of_fouth[z][j]);
                intermediate_matrix_5[i][j] += (podmatrix_1_of_therd[i][z] + podmatrix_2_of_therd[i][z]) * podmatrix_4_of_fouth[z][j];
                intermediate_matrix_6[i][j] += (podmatrix_3_of_therd[i][z] - podmatrix_1_of_therd[i][z]) * (podmatrix_1_of_fouth[z][j] + podmatrix_2_of_fouth[z][j]);
                intermediate_matrix_7[i][j] += (podmatrix_2_of_therd[i][z] - podmatrix_4_of_therd[i][z]) * (podmatrix_3_of_fouth[z][j] + podmatrix_4_of_fouth[z][j]);
            }
        }
    }
    ...
    delete_matrix(therd_matrix,size_of_intermediate_matrix);
    delete_matrix(fouth_matrix,size_of_intermediate_matrix);
    delete_matrix(prefinal_matrix,size_of_intermediate_matrix);
    delete_matrix(final_matrix,width_of_final_matrix);
    delete_matrix(podmatrix_1_of_therd,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_2_of_therd,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_3_of_therd,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_4_of_therd,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_1_of_fouth,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_2_of_fouth,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_3_of_fouth,size_of_intermediate_matrix / 2);
    delete_matrix(podmatrix_4_of_fouth,size_of_intermediate_matrix / 2);
